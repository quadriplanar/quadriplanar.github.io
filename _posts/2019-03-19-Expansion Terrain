...
A new and improved version of the terrain generation program from yesterday, this one works by the following method:
  ...
  2) Seed a **number** of land tiles randomly about
  3) Then expands these land masses by looking at each tile and turning it to land if:
        i) the number of adjacent land tiles are above a certain **threshold**
        ii) or a **RNG chance** to turn to land
  4) Step 3) is repeated a number of **times**
